...
public class RelevantOffers implements Constants {
    ...
    public RelevantOffers(Context context, Polygons polygons, String token) {
        this.context = context;
        offers = new ArrayList<>();

        RequestQueue requestQueue = Volley.newRequestQueue(context);

        final String requestBody = polygons.getBody();

//        Log.d("M2TAG", "OffersBody: " + requestBody);

        StringRequest stringRequest = new StringRequest(Request.Method.POST, URL_GET_RELEVANT_OFFERS, new Response.Listener<String>() {
            @Override
